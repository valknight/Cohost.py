...
def main():
    ...
    blocks = [
        MarkdownBlock('<div style="position: relative; top: 3px; display: inline-block; animation: 1s ease-in-out 0s infinite normal none running bounce;">cohost.py 0.4 is out!!</div>'),
        MarkdownBlock('thanks to the hard work of GitHub user [a-e-m](https://github.com/a-e-m) this update contains a contribution to read a page\'s asks from the API, with `getAsksRaw`!'),
        ...
        MarkdownBlock('<hr>'),
        MarkdownBlock('check out <a href="https://github.com/valknight/Cohost.py/blob/main/demos/releaseNotes/0.4.0.py">the source for this post</a>'),
        MarkdownBlock("<hr>"),
