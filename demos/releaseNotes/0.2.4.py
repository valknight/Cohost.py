...
def main():
    ...
    if handle is None:
        handle = input('handle: ')
    blocks = [
      MarkdownBlock('<div style="position: relative; top: 3px; display: inline-block; animation: 1s ease-in-out 0s infinite normal none running bounce;">cohost.py 0.2.4 is out!!</div>'),
        MarkdownBlock('a lot of little things today!'),
        MarkdownBlock('big news! we can now edit posts - thanks again to GitHub user CrCreate ! (also hello CrCreate if you are reading this feel free to let me know your cohost username so I can tag you here!)'),
        MarkdownBlock('second - cohost no longer does a big sad when we try to login. this was actually a yesterday thing but i forgot to do a release note, oops'),
        MarkdownBlock('for code quality, big thanks to @oliviac for removing a dependency, and lots of cleaning up of unsued imports - super appreciated!'),
        ...
        MarkdownBlock('<hr>'),
        MarkdownBlock('check out <a href="https://github.com/valknight/Cohost.py/blob/main/demos/releaseNotes/0.2.4.py">the source for this post</a>'),
        MarkdownBlock("<hr>"),
